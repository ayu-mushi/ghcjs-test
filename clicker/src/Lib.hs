-- TODO 通知 title "(1002)->(2005) Clicker"
...
import Reflex.Dom as Reflex
  ...
  MonadHold, hold, tagPromptlyDyn, textArea, textArea_value, TextArea, attributes, constDyn, (=:), textAreaConfig_initialValue, attach, attachWith, getPostBuild, PostBuild, attachWidget)
...
data Dice = Gold | Silver | Copper -- 出目
...
headWidget cookie = do
  el "title" $ dynText $ fmap (Text.pack . show) cookie
  return ()
...
someFunc :: IO ()
someFunc = mainWidgetWithHead' (headWidget, (\()->myWidget))
